...
import { EmailMessage, User } from "../models/user"
...
export class ExportService {
    ...
    docDefinition = {
      ...
      this.docDefinition.styles = {
        ...
        filterPart(parts) {
            let part = {
             treasures: parts.filter(part => part.parent == Parent.treasures).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             apply: parts.filter(part => part.parent == Parent.apply).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             life: parts.filter(part => part.parent == Parent.life).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             talk: parts.filter(part => part.parent == Parent.talk).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             wt: parts.filter(part => part.parent == Parent.wt).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             chairmans: parts.filter(part => part.parent == Parent.chairman).sort((a, b) => (a.index ?? 0) - (b.index ?? 1)),
             prayers: parts.filter(part => part.parent == Parent.prayer).sort((a, b) => (a.index ?? 0) - (b.index ?? 1))
             }
             return part;
            }
         
        pagebreak(weekProgram, weeks) {
             return weekProgram.range != weeks[weeks.length - 1].range;
            }

         downloadPartPDF(part) {
        ...
        parsePart(part) {
          ...
          emailPartPDF(part, user, congregation) {
            if (part.assignee) {
              this.parsePart(part)
          ...
          parsePDFPage(congInfo, weekProgram, parts) {
          ...
          parseSinglePage(congInfo, weekProgram, part, weeks) {
            let filteredParts = this.filterPart(parts)
